    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        {isRejected && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-600 mt-1">Customer ko re-upload ke liye notification bheja gaya hai</p>
            </div>
          </div>
        )}
            ...
            return (
              <div key={step.key} className="relative pb-8 last:pb-0">
                ...
                <div className="absolute left-[-28px] top-0.5">
                  {isCompleted ? (
                    <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center"><CheckCircle className="w-4 h-4 text-white" /></div>
                  ) : isCurrent ? (
                    <div className="w-6 h-6 rounded-full bg-accent flex items-center justify-center animate-pulse"><Circle className="w-4 h-4 text-white fill-white" /></div>
                  ) : (
                ...
                <div className={`${isCurrent ? 'opacity-100' : isCompleted ? 'opacity-80' : 'opacity-40'}`}>
                  ...
                  {historyEntry && (
                    <p className="text-xs text-gray-500 mt-0.5">
                      ...
                      {historyEntry.changer && ` — by ${historyEntry.changer.name}`}
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div><p className="text-gray-500">Name</p><p className="font-medium">{request.customer?.name}</p></div>
          <div><p className="text-gray-500">Email</p><p className="font-medium">{request.customer?.email}</p></div>
          <div><p className="text-gray-500">Phone</p><p className="font-medium">{request.customer?.phone || '-'}</p></div>
          <div><p className="text-gray-500">Price Charged</p><p className="font-medium text-primary">₹{request.price_charged}</p></div>
        </div>
        {request.is_on_behalf && (
          <div className="mt-3 p-2 bg-amber-50 rounded-lg text-amber-700 text-sm">On-behalf request — created by staff</div>
        )}
      </div>
      ...
      {canComplete && (
        <div className="card bg-gradient-to-br from-primary/5 to-transparent border-primary/20">
          ...
          <p className="text-sm text-gray-500 mb-4">Jab kaam ho jaye, request complete mark karein. Agar digital output hai to file upload karein.</p>
          <button
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
          <div><p className="text-gray-500">Tracking ID</p><p className="font-medium">{request.tracking_id}</p></div>
          <div><p className="text-gray-500">Service</p><p className="font-medium">{request.service?.name}</p></div>
          <div><p className="text-gray-500">Price</p><p className="font-medium">₹{request.price_charged}</p></div>
          <div><p className="text-gray-500">Created</p><p className="font-medium">{new Date(request.created_at).toLocaleString('en-IN')}</p></div>
          <div><p className="text-gray-500">Claimed By</p><p className="font-medium">{request.claimed_by_user?.name || 'Not claimed yet'}</p></div>
          {request.completed_at && <div><p className="text-gray-500">Completed</p><p className="font-medium">{new Date(request.completed_at).toLocaleString('en-IN')}</p></div>}
          {request.notes && <div className="col-span-full"><p className="text-gray-500">Notes</p><p className="font-medium">{request.notes}</p></div>}
        </div>
      </div>
    </div>
  );
